...
class Message{
	...
	protected function setLibrary(){
		...
		if($this->language == self::ENGLISH){
			require_once(dirname(__FILE__).'/Language/Message.en.php'); //英語クラス読み込み
		}else{
			require_once(dirname(__FILE__).'/Language/Message.jp.php'); //日本語クラス読み込み
		}
	...
	function get($define){
		$args = func_get_args();
		$message = constant("MessageLibrary::".$define);
		$replace_from = array();
		$replace_to = array();
