...
$elementtype_id = isset($_REQUEST["elementtype_id"]) ? $_REQUEST["elementtype_id"] : "";													//要素種別ID
$elementtype_name = isset($_REQUEST["elementtype_name"]) ? $_REQUEST["elementtype_name"] : "";												//要素種別名
...
if(($mode == "new" || $mode == "edit") && $action == "save" && $error == array()){
	...
	if($mode == "new"){
		...
		if(!$ElementType->insert($insertData)){
			DB::rollBack();
			Logger::error("要素種別新規追加に失敗しました。",$insertData);
			Location::redirect($redirect);
		}
		//ユーザ種別IDを取得
		$elementtype_id = $ElementType->last_insert_id();
	}

	//共通更新条件
	$where = array("elementtype_id" => $elementtype_id);						//更新条件

	//更新データ設定
	$saveData = array();
	$saveData["elementtype_name"] = $elementtype_name;							//要素種別名
	$saveData["active_flg"] = 1;												//有効

	//新規追加の場合は、IDをソート順として初期設定
	if($mode == "new"){
		$saveData["sort_no"] = $elementtype_id;
	...
	//データ更新実行
	if(!$ElementType->update($where, $saveData)){
		DB::rollBack();
		Logger::error("要素種別更新に失敗しました。",$saveData);
		Location::redirect($redirect);
	}
	...
	if($close == "on"){
		...
		//同画面に遷移する
		$redirectParam["elementtype_id"] = $elementtype_id;
	...
	//共通削除条件
	$where = array("elementtype_id" => $elementtype_id);	//削除条件

	//フォルダ種別データ削除
	if(!$ElementType->delete($where)){
		DB::rollBack();
		Logger::error("要素種別削除に失敗しました。",$where);
		Location::redirect($redirect);
	}
...
if(($mode == "edit" || $mode == "delete") && $action == ""){		//編集モードで初期表示の場合
	...
	$initData = $ElementType->getDataByParameters(array("elementtype_id" => $elementtype_id));

	if(!$initData){
		//初期表示データが取得できない場合
		Logger::error("要素種別初期表示データ取得に失敗しました。",array("elementtype_id" => $elementtype_id));
		Location::redirect("../index.php");
	}

	$elementtype_name = $initData["elementtype_name"];				//フォルダ種別名
...
<input type="hidden" name="elementtype_id" value="<?php echo htmlspecialchars($elementtype_id); //要素種別ID ?>" />
...
<ul>
<li><a href="#tabs-1" class="tabmenu">要素種別設定</a></li>
</ul>
...
<?php if($restrict["all"] >= SPConst::RESTRICT_READONLY): ?>
	...
	<th>要素種別名</th>
	<td>
	<?php echo UIParts::middleText("elementtype_name",$elementtype_name,$restrict["all"]); ?>
	</td>
	</tr>
<?php endif; ?>
</table>
