...
$elementtype_list = $ElementType->getListForSetting();

$LayoutManager = new LayoutManagerSetting();
$LayoutManager->setTitle("要素種別一覧");
...
$(function(){
	...
	$(".filelist_body").sortable({
		...
		update: function(e, ui){
			...
			for(var i = 0; i < record_arr.length ; i++){
				record_arr[i] = record_arr[i].replace(/elementtype\-/,"");
			}
			//AJAXで並び替え処理を呼び出し
			var url = "sort.php";
	...
	$('.content').contextMenu('contentMenu', {
        ...
        bindings: {
            'edit': function(t) {
            	var elementtype_id = t.id.replace(/elementtype\-/,"");
            	window.location.href = 'edit.php?elementtype_id=' + elementtype_id + '&mode=edit';
            },
            'delete': function(t) {
            	var elementtype_id = t.id.replace(/elementtype\-/,"");
            	window.location.href = 'edit.php?elementtype_id=' + elementtype_id + '&mode=delete';
            }
        }
    });
});
...
<?php
foreach($elementtype_list as $key => $value){
	echo '<tr class="content" id="elementtype-'.$value["elementtype_id"].'">';
	echo '<td><a href="edit.php?mode=edit&elementtype_id='.$value["elementtype_id"].'">'.htmlspecialchars($value["elementtype_name"]).'</a></td>';
