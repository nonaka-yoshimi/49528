...
$foldertype_list = $FolderType->getListForSetting();

$LayoutManager = new LayoutManagerSetting();
$LayoutManager->setTitle("フォルダ種別一覧");
...
$(function(){
	...
	$(".filelist_body").sortable({
		...
		update: function(e, ui){
			...
			for(var i = 0; i < record_arr.length ; i++){
				record_arr[i] = record_arr[i].replace(/foldertype\-/,"");
			}
			//AJAXで並び替え処理を呼び出し
			var url = "sort.php";
	...
	$('.content').contextMenu('contentMenu', {
        ...
        bindings: {
            'edit': function(t) {
            	var foldertype_id = t.id.replace(/foldertype\-/,"");
            	window.location.href = 'edit.php?foldertype_id=' + foldertype_id + '&mode=edit';
            },
            'delete': function(t) {
            	var foldertype_id = t.id.replace(/foldertype\-/,"");
            	window.location.href = 'edit.php?foldertype_id=' + foldertype_id + '&mode=delete';
            }
        }
    });
});
...
<?php
foreach($foldertype_list as $key => $value){
	echo '<tr class="content" id="foldertype-'.$value["foldertype_id"].'">';
	echo '<td><a href="edit.php?mode=edit&foldertype_id='.$value["foldertype_id"].'">'.htmlspecialchars($value["foldertype_name"]).'</a></td>';
