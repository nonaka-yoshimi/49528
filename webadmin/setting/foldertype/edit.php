...
$foldertype_id = isset($_REQUEST["foldertype_id"]) ? $_REQUEST["foldertype_id"] : "";														//フォルダ種別ID
$foldertype_name = isset($_REQUEST["foldertype_name"]) ? $_REQUEST["foldertype_name"] : "";													//フォルダ種別名
...
if(($mode == "new" || $mode == "edit") && $action == "save" && $error == array()){
	...
	$FolderType = new FolderType();						//フォルダ種別クラス

	//新規追加の場合データを追加
	if($mode == "new"){
		$insertData = array();
		$insertData["active_flg"] = "1";
		if(!$FolderType->insert($insertData)){
			DB::rollBack();
			Logger::error("フォルダ種別新規追加に失敗しました。",$insertData);
			Location::redirect($redirect);
		}
		//ユーザ種別IDを取得
		$foldertype_id = $FolderType->last_insert_id();
	}

	//共通更新条件
	$where = array("foldertype_id" => $foldertype_id);							//更新条件

	//更新データ設定
	$saveData = array();
	$saveData["foldertype_name"] = $foldertype_name;							//フォルダ種別名
	...
	//データ更新実行
	if(!$FolderType->update($where, $saveData)){
		DB::rollBack();
		Logger::error("フォルダ種別更新に失敗しました。",$saveData);
		Location::redirect($redirect);
	}
	...
	if($close == "on"){
		...
		//同画面に遷移する
		$redirectParam["foldertype_id"] = $foldertype_id;
	...
	//共通削除条件
	$where = array("foldertype_id" => $foldertype_id);	//削除条件

	//フォルダ種別データ削除
	if(!$FolderType->delete($where)){
		DB::rollBack();
		Logger::error("フォルダ種別削除に失敗しました。",$where);
		Location::redirect($redirect);
	}
...
if(($mode == "edit" || $mode == "delete") && $action == ""){		//編集モードで初期表示の場合
	...
	$initData = $FolderType->getDataByParameters(array("foldertype_id" => $foldertype_id));

	if(!$initData){
		//初期表示データが取得できない場合
		Logger::error("フォルダ種別初期表示データ取得に失敗しました。",array("foldertype_id" => $foldertype_id));
		Location::redirect("../index.php");
	}

	$foldertype_name = $initData["foldertype_name"];				//フォルダ種別名
...
$LayoutManager->setTitle("フォルダ種別編集");
...
<input type="hidden" name="foldertype_id" value="<?php echo htmlspecialchars($foldertype_id); //フォルダ種別ID ?>" />
...
<ul>
<li><a href="#tabs-1" class="tabmenu">フォルダ種別設定</a></li>
</ul>
...
<?php if($restrict["all"] >= SPConst::RESTRICT_READONLY): ?>
	...
	<th>フォルダ種別名</th>
	<td>
	<?php echo UIParts::middleText("foldertype_name",$foldertype_name,$restrict["all"]); ?>
	</td>
	</tr>
<?php endif; ?>
</table>
