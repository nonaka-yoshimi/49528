...
$workflowstate_id = isset($_REQUEST["workflowstate_id"]) ? $_REQUEST["workflowstate_id"] : "";												//ワークフロー状態ID
$workflowstate_name = isset($_REQUEST["workflowstate_name"]) ? $_REQUEST["workflowstate_name"] : "";										//ワークフロー状態名
...
if(($mode == "new" || $mode == "edit") && $action == "save" && $error == array()){
	...
	if($mode == "new"){
		...
		if(!$WorkFlowState->insert($insertData)){
			DB::rollBack();
			Logger::error("ワークフロー状態新規追加に失敗しました。",$insertData);
			Location::redirect($redirect);
		}
		//ユーザ種別IDを取得
	...
	//更新データ設定
	$saveData = array();
	$saveData["workflowstate_name"] = $workflowstate_name;						//ワークフロー状態名
	$saveData["active_flg"] = 1;												//有効

	//新規追加の場合は、IDをソート順として初期設定
	if($mode == "new"){
		$saveData["sort_no"] = $workflowstate_id;
	...
	//共通削除条件
	$where = array("workflowstate_id" => $workflowstate_id);	//削除条件

	//フォルダ種別データ削除
	if(!$WorkFlowState->delete($where)){
		DB::rollBack();
		Logger::error("ワークフロー状態削除に失敗しました。",$where);
		Location::redirect($redirect);
	}
...
if(($mode == "edit" || $mode == "delete") && $action == ""){		//編集モードで初期表示の場合
	...
	$initData = $WorkFlowState->getDataByParameters(array("workflowstate_id" => $workflowstate_id));

	if(!$initData){
		//初期表示データが取得できない場合
		Logger::error("ワークフロー状態初期表示データ取得に失敗しました。",array("workflowstate_id" => $workflowstate_id));
		Location::redirect("../../index.php");
	}

	$workflowstate_name = $initData["workflowstate_name"];			//ワークフロー状態名
...
<ul>
<li><a href="#tabs-1" class="tabmenu">ワークフロー状態設定</a></li>
</ul>
...
<?php if($restrict["all"] >= SPConst::RESTRICT_READONLY): ?>
	...
	<th>ワークフロー状態名</th>
	<td>
	<?php echo UIParts::middleText("workflowstate_name",$workflowstate_name,$restrict["all"]); ?>
	</td>
	</tr>
<?php endif; ?>
</table>
