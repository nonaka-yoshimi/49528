...
$workflowstate_list = $WorkFlowState->getListForSetting();

$LayoutManager = new LayoutManagerSetting();
$LayoutManager->setTitle("ワークフロー状態一覧");
...
$(function(){
	...
	$(".filelist_body").sortable({
		...
		update: function(e, ui){
			...
			for(var i = 0; i < record_arr.length ; i++){
				record_arr[i] = record_arr[i].replace(/workflowstate\-/,"");
			}
			//AJAXで並び替え処理を呼び出し
			var url = "sort.php";
	...
	$('.content').contextMenu('contentMenu', {
        ...
        bindings: {
            'edit': function(t) {
            	var workflowstate_id = t.id.replace(/workflowstate\-/,"");
            	window.location.href = 'edit.php?workflowstate_id=' + workflowstate_id + '&mode=edit';
            },
            'delete': function(t) {
            	var workflowstate_id = t.id.replace(/workflowstate\-/,"");
            	window.location.href = 'edit.php?workflowstate_id=' + workflowstate_id + '&mode=delete';
            }
        }
    });
});
...
<?php
foreach($workflowstate_list as $key => $value){
	echo '<tr class="content" id="workflowstate-'.$value["workflowstate_id"].'">';
	echo '<td><a href="edit.php?mode=edit&workflowstate_id='.$value["workflowstate_id"].'">'.htmlspecialchars($value["workflowstate_name"]).'</a></td>';
