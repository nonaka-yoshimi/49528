...
$workflow_list = $WorkFlow->getListForSetting();
...
$(function(){
	...
	$(".filelist_body").sortable({
		...
		update: function(e, ui){
			...
			for(var i = 0; i < record_arr.length ; i++){
				record_arr[i] = record_arr[i].replace(/workflow\-/,"");
			}
			//AJAXで並び替え処理を呼び出し
			var url = "sort.php";
	...
	$('.content').contextMenu('contentMenu', {
        ...
        bindings: {
            'edit': function(t) {
            	var workflow_id = t.id.replace(/workflow\-/,"");
            	window.location.href = 'edit.php?workflow_id=' + workflow_id + '&mode=edit';
            },
            'delete': function(t) {
            	var workflow_id = t.id.replace(/workflow\-/,"");
            	window.location.href = 'edit.php?workflow_id=' + workflow_id + '&mode=delete';
            }
        }
    });
});
...
<?php
foreach($workflow_list as $key => $value){
	echo '<tr class="content" id="workflow-'.$value["workflow_id"].'">';
	echo '<td><a href="edit.php?mode=edit&workflow_id='.$value["workflow_id"].'">'.htmlspecialchars($value["workflow_name"]).'</a></td>';
