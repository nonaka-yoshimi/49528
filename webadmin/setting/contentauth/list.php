...
$contentauth_list = $ContentAuth->getListForSetting();

$LayoutManager = new LayoutManagerSetting();
$LayoutManager->setTitle("コンテンツ操作権限一覧");
...
$(function(){
	...
	$(".filelist_body").sortable({
		...
		update: function(e, ui){
			...
			for(var i = 0; i < record_arr.length ; i++){
				record_arr[i] = record_arr[i].replace(/contentauth\-/,"");
			}
			//AJAXで並び替え処理を呼び出し
			var url = "sort.php";
	...
	$('.content').contextMenu('contentMenu', {
        ...
        bindings: {
            'edit': function(t) {
            	var contentauth_id = t.id.replace(/contentauth\-/,"");
            	window.location.href = 'edit.php?contentauth_id=' + contentauth_id + '&mode=edit';
            },
            'delete': function(t) {
            	var contentauth_id = t.id.replace(/contentauth\-/,"");
            	window.location.href = 'edit.php?contentauth_id=' + contentauth_id + '&mode=delete';
            }
        }
    });
});
...
<?php
foreach($contentauth_list as $key => $value){
	echo '<tr class="content" id="contentauth-'.$value["contentauth_id"].'">';
	echo '<td><a href="edit.php?mode=edit&contentauth_id='.$value["contentauth_id"].'">'.htmlspecialchars($value["contentauth_name"]).'</a></td>';
