...
$usergrouptype_list = $UserGroupType->getListForSetting();

$LayoutManager = new LayoutManagerSetting();
$LayoutManager->setTitle("ユーザグループ種別一覧");
...
$(function(){
	...
	$(".filelist_body").sortable({
		...
		update: function(e, ui){
			...
			for(var i = 0; i < record_arr.length ; i++){
				record_arr[i] = record_arr[i].replace(/usergrouptype\-/,"");
			}
			//AJAXで並び替え処理を呼び出し
			var url = "sort.php";
	...
	$('.content').contextMenu('contentMenu', {
        ...
        bindings: {
            'edit': function(t) {
            	var usergrouptype_id = t.id.replace(/usergrouptype\-/,"");
            	window.location.href = 'edit.php?usergrouptype_id=' + usergrouptype_id + '&mode=edit';
            },
            'delete': function(t) {
            	var usergrouptype_id = t.id.replace(/usergrouptype\-/,"");
            	window.location.href = 'edit.php?usergrouptype_id=' + usergrouptype_id + '&mode=delete';
            }
        }
    });
});
...
<?php
foreach($usergrouptype_list as $key => $value){
	echo '<tr class="content" id="usergrouptype-'.$value["usergrouptype_id"].'">';
	echo '<td><a href="edit.php?mode=edit&usergrouptype_id='.$value["usergrouptype_id"].'">'.htmlspecialchars($value["usergrouptype_name"]).'</a></td>';
