...
$usergrouptype_id = isset($_REQUEST["usergrouptype_id"]) ? $_REQUEST["usergrouptype_id"] : "";												//ユーザグループ種別ID
$usergrouptype_name = isset($_REQUEST["usergrouptype_name"]) ? $_REQUEST["usergrouptype_name"] : "";										//ユーザグループ種別名
...
if(($mode == "new" || $mode == "edit") && $action == "save" && $error == array()){
	...
	$UserGroupType = new UserGroupType();			//ユーザグループ種別クラス

	//新規追加の場合データを追加
	if($mode == "new"){
		$insertData = array();
		$insertData["active_flg"] = "1";
		if(!$UserGroupType->insert($insertData)){
			DB::rollBack();
			Logger::error("ユーザグループ種別新規追加に失敗しました。",$insertData);
			Location::redirect($redirect);
		}
		//ユーザ種別IDを取得
		$usergrouptype_id = $UserGroupType->last_insert_id();
	}

	//共通更新条件
	$where = array("usergrouptype_id" => $usergrouptype_id);					//更新条件

	//更新データ設定
	$saveData = array();
	$saveData["usergrouptype_name"] = $usergrouptype_name;						//ユーザグループ種別名
	$saveData["active_flg"] = 1;												//有効

	//新規追加の場合は、IDをソート順として初期設定
	if($mode == "new"){
		$saveData["sort_no"] = $usergrouptype_id;
	...
	//データ更新実行
	if(!$UserGroupType->update($where, $saveData)){
		DB::rollBack();
		Logger::error("ユーザグループ種別更新に失敗しました。",$saveData);
		Location::redirect($redirect);
	}
	...
	if($close == "on"){
		...
		//同画面に遷移する
		$redirectParam["usergrouptype_id"] = $usergrouptype_id;
	...
	//共通削除条件
	$where = array("usergrouptype_id" => $usergrouptype_id);	//削除条件

	//ユーザグループ種別データ削除
	if(!$UserGroupType->delete($where)){
		DB::rollBack();
		Logger::error("ユーザグループ種別削除に失敗しました。",$where);
		Location::redirect($redirect);
	}
...
if(($mode == "edit" || $mode == "delete") && $action == ""){		//編集モードで初期表示の場合
	...
	$initData = $UserGroupType->getDataByParameters(array("usergrouptype_id" => $usergrouptype_id));

	if(!$initData){
		//初期表示データが取得できない場合
		Logger::error("ユーザグループ種別初期表示データ取得に失敗しました。",array("usergrouptype_id" => $usergrouptype_id));
		Location::redirect("../index.php");
	}

	$usergrouptype_name = $initData["usergrouptype_name"];			//ユーザグループ種別名
...
$LayoutManager->setTitle("ユーザグループ種別編集");
...
<input type="hidden" name="usergrouptype_id" value="<?php echo htmlspecialchars($usergrouptype_id); //ユーザグループ種別ID ?>" />
...
<ul>
<li><a href="#tabs-1" class="tabmenu">ユーザグループ種別設定</a></li>
</ul>
...
<?php if($restrict["all"] >= SPConst::RESTRICT_READONLY): ?>
	...
	<th>ユーザグループ種別名</th>
	<td>
	<?php echo UIParts::middleText("usergrouptype_name",$usergrouptype_name,$restrict["all"]); ?>
	</td>
	</tr>
<?php endif; ?>
</table>
