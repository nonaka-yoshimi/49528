...
$config_schedule_mode = isset($_REQUEST["config_schedule_mode"]) ? $_REQUEST["config_schedule_mode"] : "";						//スケジュールモード
...
if(($mode == "new" || $mode == "edit") && $action == "save" && $error == array()){
	...
	DB::beginTransaction();
	//設定更新実行
	if(!$Config->set("schedule_mode", $config_schedule_mode)){
		DB::rollBack();
		Logger::error("スケジュール設定更新に失敗しました。");
		Location::redirect($redirect);
	}
...
if($action == ""){
	if($mode == "edit"){
		//設定データ取得
		$config_schedule_mode = Config::get("schedule_mode");
	}
}

$LayoutManager = new LayoutManagerAdminMain();
$LayoutManager->setTitle("スケジュール設定編集");
...
<table class="edit" cellspacing="0">
	<tr>
    <th class="w240">スケジュール設定<span class="mark orange">必須</span></th>
    <td>
    <?php if($config_schedule_mode == ""){ $checked = "checked"; }else{ $checked = ""; } ?>
    <input type="radio" name="config_schedule_mode" value="" <?php echo $checked; ?> />単体スケジュールのみ&nbsp;<br>
    <?php if($config_schedule_mode == "on"){ $checked = "checked"; }else{ $checked = ""; } ?>
    <input type="radio" name="config_schedule_mode" value="on" <?php echo $checked; ?> />複数スケジュール(スケジューラ)利用<br />
    <span class="info">cronの使用が前提となります。</span>
    </td>
    </tr>
</table>
</form>
