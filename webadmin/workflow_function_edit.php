...
$config_workflow_mode = isset($_REQUEST["config_workflow_mode"]) ? $_REQUEST["config_workflow_mode"] : "";						//ワークフローモード
...
if(($mode == "new" || $mode == "edit") && $action == "save" && $error == array()){
	...
	DB::beginTransaction();
	//設定更新実行
	if(!$Config->set("workflow_mode", $config_workflow_mode)){
		DB::rollBack();
		Logger::error("ワークフローモード更新に失敗しました。");
		Location::redirect($redirect);
	}
...
if($action == ""){
	if($mode == "edit"){
		//設定データ取得
		$config_workflow_mode = Config::get("workflow_mode");
	}
}

$LayoutManager = new LayoutManagerAdminMain();
$LayoutManager->setTitle("ワークフロー設定編集");
...
<table class="edit" cellspacing="0">
	<tr>
    <th class="w240">ワークフロー機能利用<span class="mark orange">必須</span></th>
    <td>
    <?php if($config_workflow_mode == ""){ $checked = "checked"; }else{ $checked = ""; } ?>
    <input type="radio" name="config_workflow_mode" value="" <?php echo $checked; ?> />無効&nbsp;
    <?php if($config_workflow_mode == "on"){ $checked = "checked"; }else{ $checked = ""; } ?>
    <input type="radio" name="config_workflow_mode" value="on" <?php echo $checked; ?> />有効&nbsp;
    </td>
    </tr>
</table>
</form>
