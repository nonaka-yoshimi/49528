...
foreach($content_id_list as $content_id){
	...
	//自動アーカイブ処理
	if(Config::get("archive_mode") == "publish_auto"){
		//アーカイブ実行処理
		if(!$Content->archiveContent($content_id)){
			DB::rollBack();
			Logger::error("アーカイブ処理に失敗しました。content_id:".$content_id);
			Location::redirect($redirect);
		}else{
			$logparam = array();
			...
			Logger::info("アーカイブ処理を行いました。",$logparam);
		}
	}
	...
	if(!$Content->publishContent($content_id)){
		DB::rollBack();
		Logger::error("公開処理に失敗しました。content_id:".$content_id);
		Location::redirect($redirect);
	}else{
		// ファイル非公開処理
		if($deploy_file_flg){
			$contentData = $ContentPublic->getDataByPrimaryKey($content_id);
			if($contentData){
				if(($contentData["contentclass"] == "stylesheet" && Config::get("static_stylesheet") == "on")
					...
					// 物理ファイルを配信する
					Util::deployPhysicalFile(dirname(__FILE__)."/../".$contentData["url"] , $contentData["content"]);
				}
			}
		}
		...
		$logparam["user_id"] = $session->user["user_id"];
		$logparam["name"] = $session->user["name"];
		Logger::info("公開処理を行いました。",$logparam);
	}
}
DB::commit();
