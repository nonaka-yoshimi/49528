...
$config_folder_use_flg = isset($_REQUEST["config_folder_use_flg"]) ? $_REQUEST["config_folder_use_flg"] : "";					//フォルダ機能利用フラグ
$config_folder_code_display_flg = isset($_REQUEST["config_folder_code_display_flg"]) ? $_REQUEST["config_folder_code_display_flg"] : "";	//フォルダコード表示フラグ
...
if(($mode == "new" || $mode == "edit") && $action == "save" && $error == array()){
	...
	DB::beginTransaction();
	//設定更新実行
	if(!$Config->set("folder_use_flg", $config_folder_use_flg)){
		DB::rollBack();
		Logger::error("フォルダ機能利用設定更新に失敗しました。");
		Location::redirect($redirect);
	}
	//設定更新実行
	if(!$Config->set("folder_code_display_flg", $config_folder_code_display_flg)){
		DB::rollBack();
		Logger::error("フォルダコード表示設定更新に失敗しました。");
		Location::redirect($redirect);
	}
...
if($action == ""){
	if($mode == "edit"){
		//設定データ取得
		$config_folder_use_flg = Config::get("folder_use_flg");
		$config_folder_code_display_flg = Config::get("folder_code_display_flg");
	}
}

$LayoutManager = new LayoutManagerAdminMain();
$LayoutManager->setTitle("フォルダ管理機能設定編集");
...
<table class="edit" cellspacing="0">
	<tr>
    <th class="w240">フォルダ機能利用<span class="mark orange">必須</span></th>
    <td>
    <?php if($config_folder_use_flg == "off"){ $checked = "checked"; }else{ $checked = ""; } ?>
    <input type="radio" name="config_folder_use_flg" value="off" <?php echo $checked; ?> />無効&nbsp;
	<?php if($config_folder_use_flg == "on"){ $checked = "checked"; }else{ $checked = ""; } ?>
    <input type="radio" name="config_folder_use_flg" value="on" <?php echo $checked; ?> />有効&nbsp;
    </td>
    </tr>
    <tr>
    <th>フォルダコード表示<span class="mark orange">必須</span></th>
    <td>
    <?php if($config_folder_code_display_flg == "off"){ $checked = "checked"; }else{ $checked = ""; } ?>
    <input type="radio" name="config_folder_code_display_flg" value="off" <?php echo $checked; ?> />無効&nbsp;
	<?php if($config_folder_code_display_flg == "on"){ $checked = "checked"; }else{ $checked = ""; } ?>
    <input type="radio" name="config_folder_code_display_flg" value="on" <?php echo $checked; ?> />有効&nbsp;
    </td>
    </tr>
</table>
</form>
