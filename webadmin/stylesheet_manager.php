...
//権限取得
$ope_auth_publish = $session->user["ope_auth_stylesheet_publish"];
$ope_auth_edit = $session->user["ope_auth_stylesheet_edit"];
$ope_auth_delete = $session->user["ope_auth_stylesheet_delete"];
...
$LayoutManager->setTitle("スタイルシート管理");
...
<?php endif; ?>
<?php if($session->user["admintype"] == SPConst::ADMINTYPE_SUPERVISOR || $session->user["ope_auth_stylesheet_add"]): ?>
	...
	$("#action_new").click(function(){
		...
		$("*[name=mode]").val('new');
		$("*[name=contentclass]").val('stylesheet');
